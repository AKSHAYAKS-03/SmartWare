...
public class WarehouseService : IWarehouseService
{
    ...
    public WarehouseService(IUnitOfWork uow, ICacheService cacheService, INotificationService notificationService)
    {
        _uow = uow;
    ...
    {
        bool codeExists = await _uow.Repository<Warehouse>()
            .Query().AnyAsync(w => w.Code == dto.Code);
        if (codeExists)
            throw new BusinessRuleException($"Warehouse code '{dto.Code}' is already in use.");
    ...
    public async Task<WarehouseResponseDto> UpdateWarehouseAsync(Guid warehouseId, WarehouseUpdateDto dto)
    {
        ...
        bool codeConflict = await _uow.Repository<Warehouse>()
            .Query().AnyAsync(w => w.Code == dto.Code && w.Id != warehouseId);
        if (codeConflict)
            throw new BusinessRuleException($"Warehouse code '{dto.Code}' is used by another warehouse.");
    ...
    public async Task<BinLocationResponseDto> CreateBinLocationAsync(BinLocationCreateDto dto)
    {
        ...
        var barcode = dto.Barcode ?? $"{zone.Code}-{dto.BinCode}".ToUpper();

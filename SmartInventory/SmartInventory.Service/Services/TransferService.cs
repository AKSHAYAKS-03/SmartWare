...
public class TransferService : ITransferService
{
        ...
        foreach (var item in transfer.Items)
        {
            ...
            if (srcStock != null)
            {
                ...
                if (product != null)
                {
                    if (srcStock.QuantityOnHand <= 0)
                    {
                        await _notificationService.SendOutOfStockAlertAsync(product.Id, transfer.FromWarehouseId, srcStock.QuantityOnHand);
                    }
                    else if (srcStock.QuantityOnHand <= product.SafetyStockQty)
                    {
    ...
    public async Task<TransferResponseDto> ReceiveTransferAsync(Guid transferId, TransferReceiveDto dto, Guid performedBy)
    {
        ...
        if (invalidItemIds.Any())
            throw new BusinessRuleException(
                $"The following TransferItemIds do not belong to Transfer {transfer.TransferNumber}: {string.Join(", ", invalidItemIds)}. " +
                $"Ensure all item IDs match items within this transfer.");
